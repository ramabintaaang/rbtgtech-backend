...
@section('title', 'Edit Nota #' . $invoice->invoice_number . ' - RBTGTech')

@section('content')
<div class="px-margin-desktop py-lg max-w-[1200px] mx-auto flex flex-col gap-lg" 
     x-data="{
        items: {{ Js::from($invoice->items ?? [['item' => '', 'quantity' => 1, 'unit_price' => 0]]) }},
        addItem() {
    ...
    <header class="flex flex-col md:flex-row md:items-end justify-between gap-md">
        <div>
            <div class="flex items-center gap-2 text-label-md text-on-surface-variant mb-1">
                ...
                <span class="text-primary font-bold">Edit Nota</span>
            </div>
            <h1 class="text-headline-lg font-headline-lg text-on-surface">Edit Nota: {{ $invoice->invoice_number }}</h1>
            <p class="text-body-lg font-body-lg text-on-surface-variant">Perbarui rincian item, penerima, atau status nota ini.</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.invoices.print', $invoice->id) }}" class="bg-surface-container-high hover:bg-outline-variant/40 text-on-surface rounded-full px-5 py-2.5 flex items-center gap-2 transition-all font-label-md text-label-md">
    ...
    <form method="POST" action="{{ route('admin.invoices.update', $invoice->id) }}" class="flex flex-col gap-lg">
        @csrf
        @method('PUT')
        ...
        <div class="bg-surface-container-lowest rounded-3xl p-6 sm:p-8 soft-shadow border border-outline-variant/60 flex flex-col gap-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    >
                        <option value="unpaid" {{ old('status', $invoice->status) === 'unpaid' ? 'selected' : '' }}>⏳ Belum Lunas (Unpaid)</option>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input 
                            type="text" 
                            name="billed_to" 
                            value="{{ old('billed_to', $invoice->billed_to) }}" 
                            required 
                            class="w-full pl-10 pr-4 py-2.5 bg-surface-container-low rounded-xl border border-outline-variant/50 text-on-surface font-semibold focus:outline-none focus:ring-2 focus:ring-primary/50 text-body-md"
                        >
                    </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input 
                            type="text" 
                            name="billed_phone" 
                            value="{{ old('billed_phone', $invoice->billed_phone) }}" 
        ...
        <!-- Card 3: Informasi Pembayaran & Footer Nota -->
        <div class="bg-surface-container-lowest rounded-3xl p-6 sm:p-8 soft-shadow border border-outline-variant/60 flex flex-col gap-6" x-data="{ showAdvanced: false }">
            <div class="flex items-center justify-between border-b border-outline-variant/30 pb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-body-sm text-on-surface-variant">Sesuaikan data rekening pembayaran atau kontak bila diperlukan.</p>
                    </div>
                </div>
                <button 
                    type="button" 
            ...
            <div x-show="showAdvanced" x-transition class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    <label class="block text-label-sm font-bold text-on-surface mb-1">Nama Pemilik Rekening</label>
                    <input 
                        type="text" 
                        name="account_name" 
                        value="{{ old('account_name', $invoice->account_name) }}" 
                ...
                <div>
                    ...
                    <input 
                        type="text" 
                        name="thank_you_text" 
                        value="{{ old('thank_you_text', $invoice->thank_you_text) }}" 
                        class="w-full px-3 py-2 bg-surface-container-low rounded-xl border border-outline-variant/40 text-on-surface text-body-md"
                    >
                </div>
                <div>
                    ...
                    <input 
                        type="text" 
                        name="issuer_name" 
                        value="{{ old('issuer_name', $invoice->issuer_name) }}" 
                        class="w-full px-3 py-2 bg-surface-container-low rounded-xl border border-outline-variant/40 text-on-surface text-body-md"
                    >
                </div>
                <div>
                    ...
                    <input 
                        type="text" 
                        name="issuer_contact" 
                        value="{{ old('issuer_contact', $invoice->issuer_contact) }}" 
                        class="w-full px-3 py-2 bg-surface-container-low rounded-xl border border-outline-variant/40 text-on-surface text-body-md"
                    >
                </div>
            </div>
        ...
        <div class="flex items-center justify-end gap-4 py-4">
            ...
            >
                ...
                <span>Simpan Perubahan & Tampilkan Nota</span>
            </button>
        </div>
    </form>
</div>
